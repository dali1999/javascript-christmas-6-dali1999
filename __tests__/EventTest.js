...
describe('Event 테스트', () => {
  test('checkDdayDiscountDay', async () => {
    ...
    dates.forEach((value, index) => {
      expect(Event.checkDdayDiscountDay(value)).toEqual(outputs[index]);
    });
  });
  ...
  test('checkWeekdayDiscountDay', async () => {
    ...
    // then
    dates.forEach((value, index) => {
      expect(event.checkWeekdayDiscountDay(value, orderMenus)).toEqual(outputs[index]);
    });
  });
  ...
  test('checkWeekendDiscountDay', async () => {
    ...
    // then
    dates.forEach((value, index) => {
      expect(event.checkWeekendDiscountDay(value, orderMenus)).toEqual(outputs[index]);
    });
  });
  ...
  test('checkSpecialDiscountDay', async () => {
    ...
    // then
    dates.forEach((value, index) => {
      expect(event.checkSpecialDiscountDay(value)).toEqual(outputs[index]);
    });
  });
  ...
  test('totalBenefitsFromEvents', async () => {
    ...
    // when
    const event = new Event();
    event.totalBenefitsFromEvents(date, orderMenu, giveAwayInfo);
